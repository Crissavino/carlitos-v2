...
/**
 * Daily Revenue - Combines invoices from avocode and Zoho refunds from avocodebo
 *
 * HARDENING: Filter by website_id through subscriptions table for invoices
 * Note: Zoho refunds are currently global (no website_id available)
 */
export const dailyRevenue7dQuery: QueryBuilder = (websiteId?: number): QueryDefinition => {
  // For invoices, join with subscriptions to filter by website_id
  const invoiceWebsiteJoin = websiteId ? `
    INNER JOIN avocode.subscriptions sub ON i.customer_id = sub.customer_id AND sub.website_id = ?
  ` : '';
  ...
  return {
    ...
    sql: `
      ...
      FROM (
        -- Invoices from avocode (filtered by website_id if provided)
        SELECT
        ...
        -- Zoho refunds from avocodebo (only included when no website filter)
        -- TODO: Add website filtering for Zoho refunds when mapping is available
        SELECT
