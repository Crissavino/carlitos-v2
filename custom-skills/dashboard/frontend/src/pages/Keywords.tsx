...
import { RefreshCw, Clock, AlertTriangle, TrendingUp, Filter } from 'lucide-react';
import { api, type KeywordPerformanceResult, type KeywordsSummary, type KeywordsWasteResult } from '../api/client';
...
export function Keywords() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const [keywordsData, summaryData, wasteData] = await Promise.all([
        api.getKeywords(),
  ...
  const campaigns = data ? [...new Set(data.keywords.map(k => k.campaignName))].sort() : [];

  // Filter keywords
  let filteredKeywords = data?.keywords || [];
  if (filterCampaign) {
    filteredKeywords = filteredKeywords.filter(k => k.campaignName === filterCampaign);
  }
  if (filterMatchType) {
    filteredKeywords = filteredKeywords.filter(k => k.matchType === filterMatchType);
  }
  if (showWasteOnly && waste) {
    const wasteKeywordIds = new Set(waste.keywords.map(w => w.keyword.keywordId));
    filteredKeywords = filteredKeywords.filter(k => wasteKeywordIds.has(k.keywordId));
  }
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500 flex items-center gap-2">
                ...
                {data?.dateRange || 'LAST_7_DAYS'} | {data?.totalKeywords || 0} keywords
              </div>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="mb-4 flex flex-wrap gap-4 items-center">
          ...
          <span className="text-sm text-gray-500 ml-auto">
            Showing {filteredKeywords.length} of {data?.totalKeywords || 0} keywords
          </span>
        </div>
